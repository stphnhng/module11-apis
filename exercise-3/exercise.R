...
pokemon.url <- paste0(base.url,movie)
print(pokemon.url)
...
# The response itself becomes a list while the columns are data frames in a list. 
# The list simply uses keys to store the data frames.
...
poke.moves <- mutate(poke.moves, type = c("physical","physical","electric","physical","physical","physical") )
poke.moves <- mutate(poke.moves, like = c(5,4,5,3,5,3))

# Using dplyr order the moves by rank. Higher ranking moves should be on the top of the dataframe. 

poke.moves <- arrange(poke.moves, -like)
...
pokemon.shiny <- poke.data$sprites$front_shiny
poke.picture <- download.file(pokemon.shiny, "png/pikachu_shiny.png")
